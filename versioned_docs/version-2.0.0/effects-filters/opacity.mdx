---
title: Opacity
description: Controls the opacity of an element.
slug: /opacity
---
import ClassList from '@site/src/components/ClassList/ClassList';
import ClassPreview from '@site/src/components/ClassPreview/ClassPreview';

Controls the opacity of an element.

<ClassList
  classData={[
    {
      className: 'o-10',
      properties: [
        'opacity: 0.1;'
      ]
    },
    {
      className: 'o-20',
      properties: [
        'opacity: 0.2;'
      ]
    },
    {
      className: 'o-30',
      properties: [
        'opacity: 0.3;'
      ]
    },
    {
      className: 'o-40',
      properties: [
        'opacity: 0.4;'
      ]
    },
    {
      className: 'o-50',
      properties: [
        'opacity: 0.5;'
      ]
    },
    {
      className: 'o-6',
      properties: [
        'opacity: 0.6;'
      ]
    },
    {
      className: 'o-70',
      properties: [
        'opacity: 0.7;'
      ]
    },
    {
      className: 'o-80',
      properties: [
        'opacity: 0.8;'
      ]
    },
    {
      className: 'o-90',
      properties: [
        'opacity: 0.9;'
      ]
    },
    {
      className: 'o-100',
      properties: [
        'opacity: 1;'
      ]
    },
  ]}
/>

<ClassPreview
  codeData={`<div class="o-10 bg-pink dim-14 mr-3 p-2 rad-1"></div>
    <div class="o-40 bg-pink dim-14 mr-3 p-2 rad-1"></div>
    <div class="o-80 bg-pink dim-14 mr-3 p-2 rad-1"></div>`}
  isPadded
  isCentered
  setVersion = '2.0.0'
/>