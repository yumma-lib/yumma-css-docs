---
title: Overflow
---

import ClassList from '@site/src/components/ClassList/ClassList';
import ClassPreview from '@site/src/components/ClassPreview/ClassPreview';

# Overflow

Controls what happens when the content of an element overflows its box.

<ClassList
  classData={[
    {
      className: 'ovf-auto',
      properties: [
        'overflow: auto;'
      ]
    },
    {
      className: 'ovf-c',
      properties: [
        'overflow: clip;'
      ]
    },
    {
      className: 'ovf-h',
      properties: [
        'overflow: hidden;'
      ]
    },
    {
      className: 'ovf-s',
      properties: [
        'overflow: scroll;'
      ]
    },
    {
      className: 'ovf-v',
      properties: [
        'overflow: visible;'
      ]
    },
  ]}
/>

## Auto

<ClassPreview
  codeData={`
    <div class="mb-4 ovf-auto h-12 bc-gray p-1">
        <p class="t-gray">Overflow Auto</p>
        <p>In an endless white expanse, Ren stood by a solitary tree...</p>
    </div>
  `}
  isPadded = {true}
  isCentered = {false}
  isVersion = '1.2.0'
/>

## Clip

<ClassPreview
  codeData={`
    <div class="mb-4 ovf-c h-12 bc-gray p-1">
      <p class="t-gray">Overflow Clip</p>
      <p>In an endless white expanse, Ren stood by a solitary tree...</p>
    </div>
  `}
  isPadded = {true}
  isCentered = {false}
  isVersion = '1.2.0'
/>

<div class="mb-4 ovf-c h-12 bc-gray p-1">
    <p class="t-gray">Overflow Clip</p>
    <p>In an endless white expanse, Ren stood by a solitary tree...</p>
</div>

## Hidden

<ClassPreview
  codeData={`
    <div class="mb-4 ovf-h h-12 bc-gray p-1">
        <p class="t-gray">Overflow Hidden</p>
        <p>In an endless white expanse, Ren stood by a solitary tree...</p>
    </div>
  `}
  isPadded = {true}
  isCentered = {false}
  isVersion = '1.2.0'
/>

```html
<div class="mb-4 ovf-h h-12 bc-gray p-1">
    <p class="t-gray">Overflow Hidden</p>
    <p>
      In an endless white expanse, Ren stood by a solitary tree. Above him, souls streaked like comets, fleeting and swift. The world, pure white, offered only fleeting motion. In the midst of longing, Ren found a quiet surrender, waiting in timeless silence. Each soul's trajectory told a unique story of its journey. Some soared gracefully, while others spiraled with wild exuberance. The lone tree beside him seemed to share Ren's vigil, its branches reaching skyward. Its roots, hidden beneath the pristine surface, anchored it to this enigmatic world. Ren felt a kinship with the tree, both silent witnesses to the cosmic spectacle. Patient and at peace, Ren awaited his own connection to the boundless flow of existence.
    </p>
</div>
```

## Scroll

<ClassPreview
  codeData={`
    <div class="mb-4 ovf-s h-12 bc-gray p-1">
        <p class="t-gray">Overflow Scroll</p>
        <p>In an endless white expanse, Ren stood by a solitary tree...</p>
    </div>
  `}
  isPadded = {true}
  isCentered = {false}
  isVersion = '1.2.0'
/>

```html
<div class="mb-4 ovf-s h-12 bc-gray p-1">
    <p class="t-gray">Overflow Scroll</p>
    <p>
      In an endless white expanse, Ren stood by a solitary tree. Above him, souls streaked like comets, fleeting and swift. The world, pure white, offered only fleeting motion. In the midst of longing, Ren found a quiet surrender, waiting in timeless silence. Each soul's trajectory told a unique story of its journey. Some soared gracefully, while others spiraled with wild exuberance. The lone tree beside him seemed to share Ren's vigil, its branches reaching skyward. Its roots, hidden beneath the pristine surface, anchored it to this enigmatic world. Ren felt a kinship with the tree, both silent witnesses to the cosmic spectacle. Patient and at peace, Ren awaited his own connection to the boundless flow of existence.
    </p>
</div>
```