---
title: Border Collapse
description: Controls the collapsing or separation of table borders.
slug: docs/border-collapse
---

import {
  collapse,
  separate,
} from "../../../../constants/examples/border-collapse";

Controls the collapsing or separation of table borders.

<Utility
  data={[
    {
      className: "bc-c",
      properties: ["border-collapse: collapse;"],
    },
    {
      className: "bc-s",
      properties: ["border-collapse: separate;"],
    },
  ]}
/>

### Collapse

The idea is that each table cell will have its own border.

<Live
  data={collapse}
  useCenter
  useHeight="200px"
  useMarks={["bc-c"]}
  usePadding
  useVersion="release"
/>

### Separate

The borders of the tables next to each other will be merged.

<Live
  data={separate}
  useCenter
  useHeight="200px"
  useMarks={["bc-s"]}
  usePadding
  useVersion="release"
/>

---

### Using breakpoints

Using responsive breakpoints like `sm:bc-*`, `md:bc-*`, `lg:bc-*`, and `xxl:bc-*` allows targeting specific utilities in different viewports.

```html "md:bc-c"
<div class="bc-s md:bc-c ..."></div>
```

### Using variants

Using `:hover` variants such as `h:bc-*` allows you to override elements and change their values when hovering over them.

```html "h:bc-c"
<div class="bc-s h:bc-c ..."></div>
```

../../../../constants/examples/border-collapse
